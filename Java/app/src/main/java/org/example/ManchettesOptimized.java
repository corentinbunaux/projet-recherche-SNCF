...
public class ManchettesOptimized {
            ...
            if (!visitedOutliers.contains(outlier)) {
                ...
                while (!end) {
                    ...
                    if (neighbors.size() == 1) {
                        ...
                        //         if (!visited.contains(neighbor) && ligne_reference != null && code_ligne_nextStation.contains(ligne_reference) ) {
                        //             manchette.add(neighbor);
                        //             visited.add(neighbor);
                        //             allVisited.add(neighbor);
                        //             if (!visitedOutliers.contains(neighbor)) {
                        ...
                        //             currentStation = neighbor;
                        //             foundNewStation = true;
                        //             System.out.println();System.out.println("Case 2 voisins déjà visité sur la même ligne"+ neighbor+ " "+ligne_reference);
                        //             break;
                        //         }
                        //     }
                        // }
                        ...
                        if (!foundNewStation && !notyet) {
                            List<String> code_ligne_Station = RailNetwork.getCodeLignes(currentStation);
                            System.out.println("Recherche de queue avec Code ligne station: "+code_ligne_Station+" current station:"+currentStation);
                            int max=0;
                            ...
                            for (String option_code : code_ligne_Station) { 
                                ...
                                if (ligne_reference != null) { // && !option_code.equals(ligne_reference)
                                    ...
                                    if (option_code.equals(ligne_reference)){
                                        queue=exploreStation(railNetwork, option_code, currentStation, allVisited, visited, false);
                                        System.out.println("Queue pour le même code: "+queue); 
                                    }
                                    else if (!option_code.equals(ligne_reference)){
                                        queue=exploreStation(railNetwork, option_code, currentStation, allVisited, visited, true);
                                    ...
                                    if (!queue.isEmpty()) {
                                        ...
                                        if (isPriorityQueue) { // Si la queue actuelle a priorité
                                            if (queue_max == null || !queue_max.peekLast().equals("PRIORITE") || queue.size() > max) {
                                                max = queue.size();
    ...
    private static Deque<String> exploreStation(Graph<String, String> railNetwork, String option_code, String curentStation,Set<String> allVisited, Set<String> visited, boolean priority) {
