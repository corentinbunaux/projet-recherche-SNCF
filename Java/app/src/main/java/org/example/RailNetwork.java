...
public class RailNetwork {
    ...
    public static Graph<String, String> createRailNetwork(Map<String, Point2D> positions) {
        ...
        List<Ligne> lines;
        for (Gare gare : gares) {
            garesParLigne.computeIfAbsent(gare.code_ligne, _ -> new ArrayList<>()).add(gare);
        ...
        for (Gare gare : gares) {
            if (gare.x_wgs84 < minX) minX = gare.x_wgs84;
            if (gare.x_wgs84 > maxX) maxX = gare.x_wgs84;
            if (gare.y_wgs84 < minY) minY = gare.y_wgs84;
            if (gare.y_wgs84 > maxY) maxY = gare.y_wgs84;
        }
    ...
    private static List<Ligne> mergeLines(List<Ligne> lignes) {
        ...
        for (Ligne ligne : lignes) {
            lignesParCode.computeIfAbsent(ligne.code_ligne, _ -> new ArrayList<>()).add(ligne);
        }
    ...
    private static void computeEdgesToRailNetwork(List<Ligne> linesList, Map<String, List<Gare>> garesParLigne,
        ...
        for (Ligne line : linesList) {
            ...
            List<Gare> stations = garesParLigne.get(line.code_ligne);
